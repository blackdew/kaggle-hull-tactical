...
class MyServer(InferenceServer):
    ...
    def train_if_needed(self):
        ...
        # CRITICAL: Remove last 180 days (public test) to avoid leak
        TEST_SKIP = 180
        max_train_date = data["date_id"].max() - TEST_SKIP
        train = data.loc[data["date_id"] <= max_train_date].copy()

        print(f"[INFO] Total data rows: {len(data)}")
        print(f"[INFO] Training rows (excluding last {TEST_SKIP} days): {len(train)}")
        print(f"[INFO] Max train date_id: {max_train_date}")
