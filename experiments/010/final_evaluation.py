#!/usr/bin/env python3
"""Final evaluation with 5-fold CV for best configs"""
...
def eval_fold(y_pred: np.ndarray, valid_df: pd.DataFrame, seq_len: int, k: float) -> dict:
    ...
    return {"sharpe": sharpe, "mse": mse, "strat_vol": strat_vol}


def evaluate_config(train, seq_len, d_model, nhead, num_layers, k, base_features, seed=42):
    ...
    for fold_idx, (tr_idx, va_idx) in enumerate(tscv.split(train), 1):
        ...
        model = train_model(model, train_loader, val_loader, epochs=100, lr=0.001, patience=10, device=device)
...
if __name__ == "__main__":
    ...
    # Test best configs from quick tuning + original fast test
    configs = [
        (20, 64, 2, 1, 600, "Best from quick tuning"),
        (20, 128, 4, 2, 600, "Larger model (original 0.712)"),
        (15, 64, 2, 1, 600, "Shorter context"),
        (25, 64, 2, 1, 600, "Medium context"),
    ]

    all_results = []

    print("="*80)
    print("FINAL EVALUATION - 5-Fold CV")
    print("="*80)
    ...
    for i, (seq_len, d_model, nhead, num_layers, k, desc) in enumerate(configs, 1):
        ...
        result = evaluate_config(train, seq_len, d_model, nhead, num_layers, k, base_features, seed=42)
        ...
        all_results.append({
            ...
            'sharpe_folds': str(result['sharpe_folds'])
        })

        print(f"Result: {result['sharpe_mean']:.3f} ± {result['sharpe_std']:.3f}")

    # Save and display results
    df_results = pd.DataFrame(all_results).sort_values('sharpe_mean', ascending=False)
    df_results.to_csv(RESULTS / "final_evaluation.csv", index=False)

    print("\n" + "="*80)
    print("FINAL RESULTS")
    print("="*80 + "\n")
    print(df_results[['description', 'sharpe_mean', 'sharpe_std']].to_string(index=False))

    # Best
    best = df_results.iloc[0]
    print("\n" + "="*80)
    print("BEST TRANSFORMER CONFIGURATION")
    print("="*80)
    ...
    print(f"Sharpe: {best['sharpe_mean']:.3f} ± {best['sharpe_std']:.3f}")
    print(f"\nEXP-007 Baseline: 0.749")
    improvement = (best['sharpe_mean'] / 0.749 - 1) * 100
    print(f"Improvement: {improvement:+.1f}%")

    if best['sharpe_mean'] > 0.749:
        print("\n🎉 NEW BEST! Transformer beats EXP-007!")
    elif best['sharpe_mean'] > 0.70:
        print(f"\n📊 Very close to baseline (gap: {abs(0.749 - best['sharpe_mean']):.3f})")
    else:
        print(f"\n📉 Below baseline")

    print("\n[DONE] Final evaluation completed!")
