"""
EXP-025 Phase 2: 1D U-Net with Skip Connections

U-Net encoder-decoder architecture for tabular data
"""
...
print("="*80)
print("EXP-025 Phase 2: 1D U-Net with Skip Connections")
print("="*80)
...
class EncoderBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, dropout=0.3):
        ...
        self.res_block = ResidualBlock1D(out_channels, dropout)
...
class DecoderBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, dropout=0.3):
        ...
        self.upconv = nn.ConvTranspose1d(in_channels, out_channels, kernel_size=3, stride=2, padding=1, output_padding=1)
        self.bn = nn.BatchNorm1d(out_channels)
        self.res_block = ResidualBlock1D(out_channels, dropout)
...
class UNet1D(nn.Module):
    """1D U-Net for tabular data"""
    def __init__(self, input_dim=305, channels=[32, 64, 128, 256], dropout=0.3):
        ...
        self.input_conv = nn.Sequential(
            ...
            nn.ReLU(),
            ResidualBlock1D(channels[0], dropout)
        )

        # Encoder
        self.encoder1 = EncoderBlock(channels[0], channels[1], dropout)
        self.encoder2 = EncoderBlock(channels[1], channels[2], dropout)
        self.encoder3 = EncoderBlock(channels[2], channels[3], dropout)

        # Bottleneck
        self.bottleneck = nn.Sequential(
            ResidualBlock1D(channels[3], dropout),
            ResidualBlock1D(channels[3], dropout)
        )

        # Decoder
        self.decoder3 = DecoderBlock(channels[3], channels[2], dropout)
        self.decoder2 = DecoderBlock(channels[2], channels[1], dropout)
        self.decoder1 = DecoderBlock(channels[1], channels[0], dropout)
    ...
    def forward(self, x):
        # x: (batch, 305)
        x = x.unsqueeze(1)  # (batch, 1, 305)

        # Encoder
        e0 = self.input_conv(x)       # (batch, 64, 305)
        e1 = self.encoder1(e0)         # (batch, 128, 153)
        e2 = self.encoder2(e1)         # (batch, 256, 77)
        e3 = self.encoder3(e2)         # (batch, 512, 39)

        # Bottleneck
        b = self.bottleneck(e3)        # (batch, 512, 39)

        # Decoder with skip connections
        d3 = self.decoder3(b, e2)      # (batch, 256, 77) + skip from e2
        d2 = self.decoder2(d3, e1)     # (batch, 128, 153) + skip from e1
        d1 = self.decoder1(d2, e0)     # (batch, 64, 305) + skip from e0

        # Global pooling and output
        pooled = self.global_pool(d1)  # (batch, 64, 1)
        pooled = pooled.squeeze(2)     # (batch, 64)
...
print("\n[2] Training 1D U-Net with 5-fold CV...")
...
for fold_idx, (train_idx, val_idx) in enumerate(tscv.split(X_all), 1):
    ...
    # Model - Reduced channels for faster training
    model = UNet1D(
...
# Save
output_path = Path("experiments/025/results/phase2_unet1d_results.csv")
