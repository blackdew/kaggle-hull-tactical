#!/usr/bin/env python3
"""EXP-015 Medium: Transformer with Residual - Medium size"""
...
class MediumTransformer(nn.Module):
    def __init__(self, input_dim: int = 94, d_model: int = 96, nhead: int = 3, num_layers: int = 2, seq_len: int = 35):
        super().__init__()
        self.input_proj = nn.Linear(input_dim, d_model)
        self.pos_encoding = PositionalEncoding(d_model, max_len=seq_len)
        self.transformer_blocks = nn.ModuleList([
            TransformerBlock(d_model, nhead, d_model * 3, dropout=0.1)
...
class MultivarSequenceDataset(Dataset):
    def __init__(self, X: np.ndarray, y: np.ndarray, seq_len: int = 35):
        self.X = X
        self.y = y
...
def train_model_fast(model, train_loader, val_loader, epochs=40, lr=0.001, patience=10, device="cpu"):
...
if __name__ == "__main__":
    ...
    print("Transformer + Residual (Medium): d_model=96, heads=3, layers=2, seq=35")
    print("="*80)
    ...
    for fold_idx, (tr_idx, va_idx) in enumerate(tscv.split(train), 1):
        ...
        model = MediumTransformer(input_dim=len(all_features), d_model=96, nhead=3, num_layers=2, seq_len=seq_len)
        print(f"  Parameters: {sum(p.numel() for p in model.parameters()):,}")

        model = train_model_fast(model, train_loader, val_loader, epochs=40, lr=0.001, patience=10, device="cpu")
    ...
    df_results.to_csv(RESULTS / "transformer_medium.csv", index=False)
    print("\n[DONE]")
